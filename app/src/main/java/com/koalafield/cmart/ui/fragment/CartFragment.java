...
public class CartFragment extends BaseFragment   implements ICartListView<List<CartDataBean>>,
    ...
    public void onResume() {
        super.onResume();
        Log.e("登录后是否走这里","**********************************************");
        ICartListPresenter presenter = new CartListPresenter(this);
        presenter.getData();
    }
    ...
    public void onSucessCartFul(List<CartDataBean> data) {
        if (data != null && data.size() >0 ){
            ...
            mCartBean = data;
            cartItemAdapter = new CartItemAdapter(mContext,data,goods_item_recycler);
            RecyclerViewHelper.initRecyclerViewV(mContext,goods_item_recycler,false,cartItemAdapter);
    ...
    public void onChangeItemSucessful(CartIdBean responseBean) {
        Toast.makeText(mContext,"增或减成功",Toast.LENGTH_SHORT).show();
        cartItemAdapter.updateItems(mCartBean);
        if (mCartBean == null  || mCartBean.size() == 0){
            empty_cart.setVisibility(View.VISIBLE);
            cart_top_layout.setVisibility(View.GONE);
